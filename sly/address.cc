...
// IPv6：IPv6地址由8组16位的十六进制数字组成，每组表示为4个十六进制数，
// 使用冒号分隔，例如：2001:0db8:85a3:0000:0000:8a2e:0370:7334。
    std::ostream& IPv6Address::insert(std::ostream& os) const {
        // 在输出流中插入左括号，表示 IPv6 地址的开始
        os << "[";
        uint16_t* addr = (uint16_t*)m_addr.sin6_addr.s6_addr;
        // 用于标记是否已经插入过连续的零段
        bool used_zeros = false;
        for(size_t i = 0; i < 8; ++i) {
            // 如果当前段为零且之前没有插入过连续的零段，则跳过
            if(addr[i] == 0 && !used_zeros) {
                continue;
            }
            // 如果当前段不为零，但前一段为零且之前没有插入过连续的零段，则插入一个冒号
        ...
        // 如果没有连续的零段，并且最后一段为零，则插入双冒号表示省略的零段
